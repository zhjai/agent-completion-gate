...
## 和 `/goal` 配合——补上它缺的那道外部产物检查

Claude Code 现在内置了 **`/goal`**，Codex 也有内置的 goal 机制：你设一个完成条件，agent 自主循环，直到条件被判定达成。这是个好用的自主循环——但做停止判断的，是一个**内部的、只看对话记录的评估器**（Claude Code 默认用 Haiku），不是会调用工具的验证者。Claude Code 官方文档说得很明白：那个评估器*「不调用工具，只能根据 Claude 已经在对话里说出来的内容来判断」*——它读的是**对话记录**，不是你的真实文件。

这正是本 gate 要补的缺口。`/goal` 决定*什么时候继续跑*；gate 决定*什么才算做完*——靠读真实产物。两者通过对话记录组合起来：

- **让完成条件依赖 gate 的证据。** agent 在自己这一轮里跑 `gate/verify_completion.sh` 并把结果说出来；然后把 `/goal` 的条件设成*「只有当对话里出现一次成功的 gate 运行，才算完成」*。这样循环会一直跑，直到一道读真实产物的外部检查**真的通过并被展示出来**——而不是叙述听起来*像*做完了就停。
- `goal-compile`（本 kit 的 skill）就是这条路的入口：它把你的目标编译成验收标准，让你确认一次，然后跑 gate——于是当 goal 的完成条件要求「对话里出现成功的 gate 运行」时，循环就终止于一次真实验证步骤的记录证据，而不是叙述式的自我评估。

内置的 goal 循环给你自主循环；gate 给这个循环一道有真实产物支撑的检查来作为停止依据。两个一起用。
...
`v0.4.3` 预览版。MIT。Agent 无关、基于文件、fail-closed。可选搭档：[`agent-lessonbook`](https://github.com/zhjai/agent-lessonbook)。

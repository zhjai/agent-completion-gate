...
Worker 只能到 `candidate_complete` 或 `blocked`。**只有外部验证者写 `complete`。** **`needs-review == blocked`**（不是 agent 设一下就能继续往前走的注解）。本 kit 交付**检查 + 契约 + 接线**：`check_acceptance.py` 返回裁决；[`gate/verify_completion.sh`](gate/verify_completion.sh) 在它外面强制状态机（拒绝自己写 `complete` 的 worker；只有干净通过才授予 `complete`）；[`integrations/`](integrations/) 把它接成 **CI 必需检查**（权威）、**agent Stop-hook**、以及 **pre-push** hook。见 [`STATE_MACHINE.md`](STATE_MACHINE.md)。
...
3. **未知项 fail closed** —— 一个被改动过的用户可见 surface，若没有任何通过的检查 → blocked。（默认读取 worker 自报的 `touched_surfaces`；用 `--strict-surfaces`、或喂一个由 diff 推导出的 candidate，让这条规则不必信任 worker。）
...
`v0.2.0` 预览版。MIT。Agent 无关、基于文件、fail-closed。地基：[`agent-memory`](https://github.com/zhjai/agent-memory)。
